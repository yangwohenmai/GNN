...
earlyStopPatience = 3000     # 连续多少轮验证F1未提升则停止
...
def save_trained_model(model, dataDate, mode, save_dir):
    """
    训练完成后保存模型权重
    :param model: 训练好的模型
...
def run_all_func():
    ...
    # 数据采集完成，立即登出baostock（后续训练/评估/可视化不需要baostock连接）
    bs.logout()
...
def run_method_three(stock_list_multi=None, compare_modes_multi=None, ifSaveModel=False):
    ...
    if len(compare_modes_multi) > 1:
        ...
        for mode in compare_modes_multi:
            ...
            if ifSaveModel:
                save_trained_model(result_multi['model'], dataDate, mode, modelSaveDir)
        print(f'\n========== 对比结果 (方式三-多股票拼大图) ==========')
        print(f'{"模式":<10}{"Acc":>9}{"Precision":>11}{"Recall":>9}{"F1":>9}{"valF1":>9}{"轮次":>7}{"耗时(s)":>10}')
        print('-' * 75)
...
def run_method_four(model_name, net_mode, stock_list=None):
    ...
    data, _, _, test_mask = build_graph(stock_data_list, cfg)
    model = Net(cfg).to(device)
...
if __name__ == '__main__':
    ...
    #run_method_three(compare_modes_multi=['mixed', 'onlyGAT', 'onlyGCN'], ifSaveModel=True)  # 对比多种网络模式，训练完保存模型
    ...
    #run_method_four(model_name='20250101_mixed', net_mode='mixed', stock_list=['000009.SZ', '000010.SZ'])  # 手动指定股票，用指定股票的数据做预测