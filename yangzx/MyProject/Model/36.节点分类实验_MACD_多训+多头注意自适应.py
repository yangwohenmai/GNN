...
class Net(torch.nn.Module):
    def __init__(self, cfg):
        ...
        # 动态维度推导：concat=True时GAT层输出维度变化，最后一层强制2维
        heads = cfg.get('numAttentionHeads', 1)
        concat = cfg.get('ifOpenGATConcat', ifOpenGATConcat)
        actual_dims = []  # [(in_dim, out_dim), ...]
        cur_dim = feat_dim
        for i, (_, target_out) in enumerate(dims):
            in_d = cur_dim
            if i == len(dims) - 1:
                out_d = 2  # 最后一层强制2维
            elif self.netMode == 'onlyGCN':
                out_d = target_out
        ...
        self.is_gat = []
        for i, (in_d, out_d) in enumerate(actual_dims):
            is_last = (i == len(actual_dims) - 1)
            if self.netMode == 'onlyGCN':
                ...
                if i % 2 == 0:  # GCN层
                    ...
                    if concat:
                        # 维度保持型concat：每头输出out_d//heads，拼接后=out_d
                        per_head_out = out_d // heads
                        layer = GATConv(in_d, per_head_out, heads=heads, concat=True)
                    else:
                        layer = GATConv(in_d, out_d, heads=heads, concat=False)
                    self.is_gat.append(True)
        ...
        # 投影层动态创建（维度相同时用Identity零开销，不同时用Linear投影对齐）
        # proj1: 历史注入（feat_dim * residualHistoryN → conv1实际输出）
        self.proj1 = torch.nn.Linear(feat_dim * cfg['residualHistoryN'], actual_dims[0][1])
        # proj3: conv2输出 → conv3输出
        d2_out, d3_out = actual_dims[1][1], actual_dims[2][1]
        self.proj3 = torch.nn.Identity() if d2_out == d3_out else torch.nn.Linear(d2_out, d3_out)
        # proj5: conv4输出 → conv5输出
        d4_out, d5_out = actual_dims[3][1], actual_dims[4][1]
        self.proj5 = torch.nn.Identity() if d4_out == d5_out else torch.nn.Linear(d4_out, d5_out)
        # proj9: conv8输出 → conv9输出
        d8_out, d9_out = actual_dims[7][1], actual_dims[8][1]
        self.proj9 = torch.nn.Identity() if d8_out == d9_out else torch.nn.Linear(d8_out, d9_out)
        # BatchNorm动态创建（根据actual_dims每层实际输出维度）
        self.ifOpenBatchNorm = cfg['ifOpenBatchNorm']
        if self.ifOpenBatchNorm:
            for i in range(9):  # bn1~bn9（conv10不加BN）
                setattr(self, f'bn{i+1}', torch.nn.BatchNorm1d(actual_dims[i][1]))
...
def save_trained_model(model, dataDate, mode, save_dir, scaler=None, stock_count=None, accuracy=None, stop_prefix='', periodRange=None, edgeWindowK=None, edgeStride=None, residualHistoryN=None, cfg=None):
    ...
    示例：
      - 逐股训练：20260821_000001.SZ_onlyGAT_(1400d,21,3,5r,4h,tw)_72.34.pth
      - 拼大图训练：20260821_onlyGAT_300s_(1400d,21,3,5r,4h,fw)_75.61.pth
      - 中断保存：20260821_onlyGAT_300s_(1400d,21,3,5r,1h,fw)_stop68.52.pth
    ...
    :param cfg: 超参数字典，用于读取numAttentionHeads和ifOpenGATConcat
    """
    ...
    # 拼接数据配置参数：(1400d,21,3,5r,{heads}h,{concat}w)
    config_parts = []
    ...
    if cfg is not None:
        heads = cfg.get('numAttentionHeads', 1)
        config_parts.append(f'{heads}h')
        concat = cfg.get('ifOpenGATConcat', ifOpenGATConcat)
        config_parts.append('tw' if concat else 'fw')
    if config_parts:
    ...
    if cfg is not None:
        ...
        save_dict['ifOpenGATConcat'] = cfg.get('ifOpenGATConcat', ifOpenGATConcat)
...
def run_all_func_lite(modes):
    ...
    :param modes: 网络模式列表，如 ['onlyGAT'] 只跑一种，或 ['mixed', 'onlyGCN', 'onlyGAT'] 跑多种对比；必填
    日志：运行期间本函数输出到控制台的所有信息，经log_print原样追加写入 超参数_{dataDate}_{maxStockCount}s_({periodRange}d,{len(modes)}m,{heads_tag},{hyperSearchTrials}round).txt（concat=False时显示mhead，concat=True时显示wmhead）
    """
    global current_code, allStockSorted, _active_log_file
    # 日志文件名：concat=True时显示wmhead，concat=False时显示mhead
    heads_tag = 'wmhead' if ifOpenGATConcat else 'mhead'
    # 打开日志文件：本函数控制台输出内容原样写入txt（追加模式，每行写入后立即flush防中断丢失）
    _active_log_file = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), f'超参数_{dataDate}_{maxStockCount}s_({periodRange}d,{len(modes)}m,{heads_tag},{hyperSearchTrials}round).txt'), 'a', encoding='utf-8')
    if not useLocalData:
        bs.login()
    stock_data_list = []
...
def run_method_four(model_name, net_mode, stock_list=None):
    ...
    cfg = {
        ...
        'ifOpenGATConcat': checkpoint.get('ifOpenGATConcat', False),
        'netMode': net_mode,
    }
    if scaler is not None:
        print(f'已加载归一化参数')
