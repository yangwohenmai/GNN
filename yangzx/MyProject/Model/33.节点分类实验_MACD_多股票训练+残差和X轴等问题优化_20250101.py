...
def run_all_func_lite(modes):
    ...
    :param modes: 网络模式列表，如 ['onlyGAT'] 只跑一种，或 ['mixed', 'onlyGCN', 'onlyGAT'] 跑多种对比；必填
    日志：运行期间本函数输出到控制台的所有信息，经log_print原样追加写入 超参数_{dataDate}.txt
    """
    global current_code, allStockSorted, _active_log_file
    # 打开日志文件：本函数控制台输出内容原样写入txt（追加模式，每行写入后立即flush防中断丢失）
    _active_log_file = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), f'超参数_{dataDate}.txt'), 'a', encoding='utf-8')
    if not useLocalData:
        bs.login()
    stock_data_list = []
