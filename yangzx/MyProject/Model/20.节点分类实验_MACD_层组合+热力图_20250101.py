...
edgeWindowK =8            # 入边窗口大小（每个节点i接收前K个相邻节点的边X[i-K]~X[i-1]→X[i]，1=单链结构）
edgeStride = 2              # 入边稀疏间隔（从X[i-1]开始每隔stride取一个，如K=3、stride=2时仅X[i-3]、X[i-1]→X[i]，1=稠密窗口）
...
def plot_attention_heatmaps(model, data, priceDic, cfg, train_mask, val_mask, stock_code='', mode=''):
    ...
    crop_start = K - 1
    layer_mats = [mat[:, crop_start:] for mat in layer_mats]
    N_cropped = N - crop_start
    # 横轴日期刻度与 train/val/test 分界位置（裁剪后）
    dates = list(priceDic.keys())[crop_start:]
    ...
    axes[0].set_title(f'{stock_code} 各层热力图（GAT层=注意力权重α，GCN层=特征相似度伪注意力，白线=val起点，红线=test起点）')

    # 图2：收盘价/标签(上) + 多层平均热力图(下)，对齐时间轴观察注意力突变与行情/信号的关系
    mean_mat = np.nanmean(np.stack(layer_mats), axis=0)
    closes = [float(v['close']) for v in priceDic.values()][crop_start:]  # 裁剪对齐
    y_np = data.y.cpu().numpy()[crop_start:]  # 裁剪对齐
    ...
    ax1.set_title(f'{stock_code} 收盘价/标签(上) 与 多层平均注意力(下) 对齐视图（蓝线=val/test分界）')
    ...
    f1_name = f'{ts}_{mode}_GAT层热力图_分层_{stock_code}.png'
    f2_name = f'{ts}_{mode}_GAT层热力图_价格对齐_{stock_code}.png'
...
def plot_gcn_implicit_attention_heatmap(model, data, priceDic, cfg, train_mask, val_mask, stock_code='', mode=''):
    ...
    for ax, mat, name in zip(axes, layer_mats, gcn_layer_names):
        ...
        ax.set_yticks(lag_ticks)
        ax.set_yticklabels([str(l + 1) for l in lag_ticks])
        ax.axvline(split_train, color='white', linestyle='--', linewidth=1)
        ax.axvline(split_val, color='red', linestyle='--', linewidth=1)
        fig1.colorbar(im, ax=ax, pad=0.01)
    axes[-1].set_xticks(tick_pos)
    axes[-1].set_xticklabels([dates[p] for p in tick_pos], rotation=30)
    axes[0].set_title(f'{stock_code} 各GCN层伪注意力热力图（纵轴=向前看第几天lag，颜色=特征相似度归一化权重，白线=val起点，红线=test起点）')
    ...
    ax1.set_ylabel('close')
    ax1.legend(loc='upper left', fontsize=8)
    ax1.set_title(f'{stock_code} 收盘价/标签(上) 与 GCN多层平均伪注意力(下) 对齐视图（蓝线=val/test分界）')
    im2 = ax2.imshow(mean_mat, aspect='auto', origin='lower', cmap=cmap, interpolation='nearest')
    ax2.set_ylabel('lag(天)')
    ax2.set_yticks(lag_ticks)
    ax2.set_yticklabels([str(l + 1) for l in lag_ticks])
    ...
    # 保存和显示
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    f1_name = f'{ts}_{mode}_GCN伪注意力热力图_分层_{stock_code}.png'
    f2_name = f'{ts}_{mode}_GCN伪注意力热力图_价格对齐_{stock_code}.png'
    fig1.savefig(f1_name, dpi=150)
    fig2.savefig(f2_name, dpi=150)
    print(f'GCN伪注意力热力图已保存: {f1_name} / {f2_name}')
    plt.show()
