...
class Net(torch.nn.Module):
    def __init__(self, cfg):
        ...
        # ====== 冻结GAT层的目标端注意力参数att_dst ======
        # GATConv注意力系数 alpha = alpha_j + alpha_i，其中 alpha_i = (x_dst * att_dst).sum()
        # 目标节点自身特征通过 att_dst 参与注意力打分，当特征包含标签信息时模型会"抄答案"
        # 冻结 att_dst 为零向量后，alpha_i 恒为0，注意力仅依赖邻居特征，切断泄漏通道
        for i in range(len(dims)):
            layer = getattr(self, f'conv{i+1}')
            if self.is_gat[i]:
                layer.att_dst.data.zero_()
                layer.att_dst.requires_grad = False
