...
printInterval = 10   #训练参数打印间隔
ifOpenNormalize = False #是否启用归一化（不开）
ifOpenEarlyStop = False    #是否启用早停（不开）
earlyStopPatience = 1000   #连续多少轮验证F1未提升则停止
...
stockPriceDic = StockData.GetStockPriceDWMBaostock('000001.SZ', "20250501", 1400)
...
def normalize_features(data, train_mask):
    ...
    scaler.fit(x_np[train_mask, :5])            # 只用训练集 fit 前5列
