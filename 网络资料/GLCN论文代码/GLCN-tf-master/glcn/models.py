...
class SGLCN(object):
    ...
    def _loss(self):
        ...
        # Graph Learning loss
        D = tf.compat.v1.matrix_diag(tf.ones(self.placeholders['num_nodes']))*-1
        D = tf.compat.v1.sparse_add(D, self.S)*-1
        D = tf.matmul(tf.transpose(self.x), D)
        self.loss1 += tf.compat.v1.trace(tf.matmul(D, self.x)) * FLAGS.losslr1
        self.loss1 -= tf.compat.v1.trace(tf.compat.v1.sparse_tensor_dense_matmul(tf.compat.v1.sparse_transpose(self.S), tf.compat.v1.sparse_tensor_to_dense(self.S))) * FLAGS.losslr2
    ...
    def build(self):
        ...
        self.vars1 = tf.compat.v1.trainable_variables()[0:2]
        self.vars2 = tf.compat.v1.trainable_variables()[2:]
